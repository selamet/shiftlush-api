...
| Türkçe metinlerin yeri | 2.1 ve 10.3: e-posta gövdeleri `gettext` + `locale/tr/LC_MESSAGES/django.po`, QR etiketi `{% trans %}` | **İkisi de şablonda doğrudan Türkçe**; `locale/` boş, katalog yok | Faz 1'de tek dil var, dolayısıyla katalogdan çıkan tek şey bakım. Bu satır önce yalnızca etiket şablonunu kaydediyordu; aynı ödünç e-posta şablonlarında da yapılmıştı ve kaydedilmemişti — ikisi birleştirildi. Bedel somut: imaja `gettext` (msgfmt) paketi ve bir `compilemessages` adımı, CI'da aynısı, pytest'ten önce yine aynısı. **Asıl mesele:** derlenmemiş ya da bayat `.mo` hata vermez, Django msgid'e düşer — yani müşteriye **İngilizce** e-posta gider ve `tests/test_email_templates.py` bunu görmez, çünkü o testler doctype, önizleme metni, Outlook düğmesi, palet ve bağlantı gibi yapı denetliyor, Türkçe kelime denetlemiyor. Tek dilli bir üründe sessizce yanlış dile düşen bir yol kazanç değil, yeni bir risk. Şablonlar da çıkarmaya elverişli değil: `{% include "email/_button.html" with label="Hesabımı oluştur" %}` düz yazıyı şablon argümanı olarak geçiyor, `_notice.html` ise tek cümleyi satır içi `{% if single_use %}` etrafında kuruyor — ikisi de `blocktrans` ile yeniden yazılmayı gerektirir, üç ortak parça dahil on üç dosyada. İkinci dil geldiğinde yapılacak iş budur. **Bu sapmayı yapmıyormuş gibi davranan kod kaldırıldı:** `core/mail.py` ve `apps/elevators/labels.py` içindeki `translation.override("tr")` çağrıları hiçbir şeye etki etmiyordu (`LANGUAGE_CODE` zaten `"tr"`, şablonlarda çevrilecek etiket yok) ve `config/settings/base.py` metinlerin `locale/tr`'de durduğunu yazıyordu. `LOCALE_PATHS` duruyor: katalog geldiğinde gideceği yer orası |
| `/schema/` üretimde | 8.6 envanterinde kısıtsız listeleniyor | **Yalnızca `DEBUG`** — `/docs/` ile birlikte kapalı | 8.13'ün `/docs/` için verdiği gerekçe ("şema tüm alan adlarını ve iş kurallarını ifşa eder") burada geçerli değil: depo **public** ve `main`'deki `openapi/v1.yaml` aynı belgenin herkese açık duran hâli — aşağıdaki "Sözleşme kayması kontrolü" satırındaki çözüm tam olarak buna dayanıyor. Yani kapatmakla gizlenen bir şey yok. Gerekçe **okuyucusunun olmaması ve iki bedeli**: frontend sözleşmeyi artık çalışan bir backend'den değil, `npm run api:sync` ile yayımlanmış dosyadan çekiyor — 14.1'in "frontend derlemesi çalışan backend'e bağımlı olmasın" kuralı zaten bunu şart koşuyor. Bedeller: (1) CI'ın hâlihazırda denetlediği belgenin ikinci bir kopyası, dağıtım `main`'in gerisine düştüğü anda onunla çelişmekte serbest — oysa 2.6 sözleşmenin tek otoritesini `openapi/v1.yaml` yapıyor; (2) `SpectacularAPIView` şemayı her istekte tüm serializer'ları gezerek üretiyor: kimlik istemeyen ve gerçek iş yapan bir uç, üç worker'lı ve kutuyu dört uygulamayla paylaşan bir konteynerde, ucuz istekler için ölçülmüş 20/dk/IP limitinin arkasında. Üretimde sunmak önbellek ve kısıt eklemek demekti; varılacak yer, zaten var olan statik dosyanın daha yavaş bir kopyası. Geliştirmede ikisi de duruyor. `config/urls.py`'deki "üretimde IP kısıtlı" yorumu da düzeltildi — öyle bir kısıt hiçbir zaman yazılmamıştı |
| `X-Request-ID`'yi istemcinin üretmesi | 8.9 "İstemci üretir; yoksa sunucu üretir" | Gelen kimlik **biçimi doğrulanarak** kabul edilmeye devam ediyor, ama `x-request-id` **`CORS_ALLOW_HEADERS`'ta değil** — yani tarayıcı üretemiyor | Aynı 8.9 bölümü kimliğe ikinci bir görev veriyor: kullanıcı ekrandaki numarayı söylediğinde log'da **tek bir arama** o isteği bulacak. Kimliği istemci seçebiliyorsa bu iki cümle çelişir — her isteğinde aynı değeri gönderen bir istemcide arama her şeyi döndürür, başkasının kimliğini gönderen bir istemcide başkasının isteğini. Kimlik ayrıca Sentry'de bir **tag**, yani aranabilir bir dizin anahtarı; onu üçüncü tarafta istemciye seçtirmek ucuz bir karar değil. Kimliği yazabilen tek hop Caddy: DEPLOYMENT.md'de adı geçiyor ve `TRUSTED_PROXY_COUNT` istemci adresi için zaten ona güveniyor. Tarayıcı kimliği **okuyabiliyor** — `CORS_EXPOSE_HEADERS`'a eklendi, asıl hata oydu: başlık her yanıtta gönderiliyordu ama farklı origin olduğu için tarayıcıdan gizliydi, dolayısıyla frontend'in kullanıcıya gösterebildiği tek kimlik hata gövdesinden çıkanıydı ve başarılı yanıtlar izsiz kalıyordu. Bir istemcinin gerçekten kendi izini başlatması gerekirse bu tek satırlık bir değişiklik; onu güvenli kılan doğrulama (`[0-9a-fA-F][0-9a-fA-F_-]{7,63}`) şimdiden yerinde, çünkü CORS tarayıcının kapısıdır — sunucu tarafı istemciler bu yola zaten erişiyordu ve gönderdikleri şey doğrulanmadan log kaydına ve Sentry tag'ine gidiyordu |
...
| Sözleşme kayması kontrolü | 14.1 — `make sync-spec` spec'i frontend deposuna **kopyalar** ve `openapi/v1.sha256` yazar; frontend CI'ı bu iki dosyayı karşılaştırır | Sağlama toplamı dosyası **kaldırıldı**; frontend kendi kopyasını `shiftlush-api`'nin **main dalındaki yayımlanmış** spec'iyle karşılaştırıyor (`npm run lint:contract`, fail-closed) ve `npm run api:sync` ile **kendisi çekiyor** | Karşılaştırmanın iki tarafı da frontend deposundaydı ve ikisini de aynı `make sync-spec` komutu yazıyordu: çift, ne kadar geride olursa olsun kendisiyle uyuşuyordu. Elle düzenlenmiş spec'i yakalıyordu, backend'in senkron yapılmadan ilerlemesini — yani 14.1'in onu yazdırma sebebini — asla yakalayamazdı. Nitekim yakalamadı: iki kopya 214 satır ve dört `/auth` ucu ayrışmışken iki hat da yeşildi. Karşılaştırmanın bir tarafı kopyalama adımının **yukarısından** gelmek zorunda. `shiftlush-api` public olduğu için main'deki spec zaten HTTPS üzerinden duran statik bir dosya: jeton yok, klon yok, **çalışan backend yok** — 14.1'in "frontend derlemesi çalışan backend'e bağımlı olmasın" kuralı korunuyor, derleme hâlâ commit edilmiş kopyadan yapılıyor. Denetim **fail-closed**: hedef okunamazsa bu bir başarısızlık, geçiş değil. Senkronun yönü de tersine döndü — ihtiyacı olan depo çekiyor; yan yana klon ve "birinin dizüstünde çalıştırması" şartları kalktı, çünkü atlanınca iz bırakmayan şey tam olarak buydu. Backend tarafında denetim **kapı değil bildirim**: backend PR'ında frontend zorunlu olarak geridedir, orada kapı kurmak her backend değişikliğini henüz yapılamayacak bir senkrona kilitlerdi |
